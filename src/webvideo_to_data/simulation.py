...
from .config import CollisionConfig, ControlConfig, IKConfig
from .ik import JointControlProgram
...
def _set_joint_control(
    data: mujoco.MjData,
    ids: PandaSceneIds,
    program: JointControlProgram,
    sample_index: int,
) -> None:
    """Write one precomputed B0 sample to official Panda actuators 1--8."""

    data.ctrl[list(ids.arm_actuator_ids)] = program.arm_qpos_targets[sample_index]
    data.ctrl[ids.gripper_actuator_id] = program.gripper_ctrl[sample_index]
...
class SimulationResult:
    ...
    phase: tuple[str, ...]
    forbidden_contact: NDArray[np.bool_]
    maximum_penetration_m: NDArray[np.float64]
    bilateral_contact: NDArray[np.bool_]
    box_support_contact: NDArray[np.bool_]
    tcp_position_within_tolerance: NDArray[np.bool_]
    tcp_orientation_within_tolerance: NDArray[np.bool_]
    joint_position_violation: NDArray[np.bool_]
    joint_velocity_violation: NDArray[np.bool_]
    joint_acceleration_violation: NDArray[np.bool_]
    valid_numerical_state: NDArray[np.bool_]
    can_tilt_rad: NDArray[np.float64]
    can_linear_speed_m_s: NDArray[np.float64]
...
def _quaternion_geodesic_error(
    actual_wxyz: NDArray[np.float64], desired_wxyz: NDArray[np.float64]
) -> float:
    actual = actual_wxyz / np.linalg.norm(actual_wxyz)
    desired = desired_wxyz / np.linalg.norm(desired_wxyz)
    return float(2.0 * np.arccos(np.clip(abs(np.dot(actual, desired)), 0.0, 1.0)))


def _can_tilt(quaternion_wxyz: NDArray[np.float64]) -> float:
    rotation = np.empty(9, dtype=float)
    mujoco.mju_quat2Mat(rotation, quaternion_wxyz)
    local_z_world = rotation.reshape(3, 3)[:, 2]
    return float(np.arccos(np.clip(local_z_world[2], -1.0, 1.0)))


def _longest_true_duration(values: NDArray[np.bool_], timestep: float) -> float:
    longest = current = 0
    for value in values:
...
def run_joint_control_program(
    ...
    *,
    ...
    """Execute a precomputed Panda program and record measured safety evidence."""

    if render_every <= 0 or min(render_size) <= 0:
        raise ValueError("render interval and dimensions must be positive")
    timestep = float(model.opt.timestep)
    requested_steps = (
        int(np.ceil(float(program.timestamps_s[-1]) / timestep)) + 1
    ...
    timestamps = np.empty(requested_steps, dtype=float)
    control_history = np.empty((requested_steps, model.nu), dtype=float)
    qpos_history = np.empty((requested_steps, model.nq), dtype=float)
    qvel_history = np.empty((requested_steps, model.nv), dtype=float)
    can_pose = np.empty((requested_steps, 7), dtype=float)
    tcp_position = np.empty((requested_steps, 3), dtype=float)
    tcp_quaternion = np.empty((requested_steps, 4), dtype=float)
    contact_count = np.empty(requested_steps, dtype=np.int64)
    forbidden_contact = np.empty(requested_steps, dtype=bool)
    maximum_penetration = np.empty(requested_steps, dtype=float)
    bilateral_contact = np.empty(requested_steps, dtype=bool)
    support_contact = np.empty(requested_steps, dtype=bool)
    position_within = np.empty(requested_steps, dtype=bool)
    orientation_within = np.empty(requested_steps, dtype=bool)
    position_violation = np.empty(requested_steps, dtype=bool)
    velocity_violation = np.empty(requested_steps, dtype=bool)
    acceleration_violation = np.empty(requested_steps, dtype=bool)
    valid_state = np.empty(requested_steps, dtype=bool)
    can_tilt = np.empty(requested_steps, dtype=float)
    can_linear_speed = np.empty(requested_steps, dtype=float)
    position_error = np.empty(requested_steps, dtype=float)
    orientation_error = np.empty(requested_steps, dtype=float)
    phases: list[str] = []
    initial_can_z = float(data.xpos[ids.can_body_id, 2])

    arm_joint_ids = np.asarray(ids.arm_joint_ids, dtype=int)
    arm_qpos_addresses = model.jnt_qposadr[arm_joint_ids]
    arm_dof_addresses = model.jnt_dofadr[arm_joint_ids]
    previous_arm_velocity = data.qvel[arm_dof_addresses].copy()
    lower = model.jnt_range[arm_joint_ids, 0]
    upper = model.jnt_range[arm_joint_ids, 1]
    can_dof_address = int(model.jnt_dofadr[ids.can_joint_id])
    finger_qpos_addresses = model.jnt_qposadr[np.asarray(ids.finger_joint_ids)]
    ...
    try:
        for step in range(requested_steps):
            sample = min(
                len(program.timestamps_s) - 1,
                max(
                    0,
                    int(
                        np.searchsorted(
                            program.timestamps_s, float(data.time), side="right"
                        )
                        - 1
                    ),
                ),
            )
            phase = program.phase[sample]
            _set_joint_control(data, ids, program, sample)
            ...
            finite = bool(
                ...
                and np.isfinite(data.site_xpos[ids.tcp_site_id]).all()
                and np.isfinite(data.site_xmat[ids.tcp_site_id]).all()
                and np.isfinite(data.xpos[ids.can_body_id]).all()
                and np.isfinite(data.xquat[ids.can_body_id]).all()
            )
            if not finite:
                raise InvalidNumericalStateError(float(data.time))

            timestamps[step] = data.time
            control_history[step] = data.ctrl
            qpos_history[step] = data.qpos
            qvel_history[step] = data.qvel
            can_pose[step, :3] = data.xpos[ids.can_body_id]
            can_pose[step, 3:] = data.xquat[ids.can_body_id]
            tcp_position[step] = data.site_xpos[ids.tcp_site_id]
            mujoco.mju_mat2Quat(
                tcp_quaternion[step], data.site_xmat[ids.tcp_site_id]
            )
            phases.append(phase)

            observation = observe_contacts(
                model, data, ids, phase, collision_config
            )
            contact_count[step] = observation.contact_pair_count
            forbidden_contact[step] = observation.has_forbidden_contact
            maximum_penetration[step] = observation.maximum_penetration_m
            bilateral_contact[step] = (
                observation.bilateral_fingertip_can_contact
            )
            support_contact[step] = observation.has_box_support_contact

            position_error[step] = np.linalg.norm(
                tcp_position[step] - program.ee_positions[sample]
            )
            orientation_error[step] = _quaternion_geodesic_error(
                tcp_quaternion[step], program.quaternion_wxyz[sample]
            )
            position_within[step] = position_error[step] <= ik.position_tolerance_m
            orientation_within[step] = (
                orientation_error[step] <= ik.orientation_tolerance_rad
            )

            arm_position = data.qpos[arm_qpos_addresses]
            arm_velocity = data.qvel[arm_dof_addresses]
            position_violation[step] = bool(
                np.any((arm_position < lower) | (arm_position > upper))
            )
            velocity_violation[step] = bool(
                np.any(
                    np.abs(arm_velocity)
                    > control_config.maximum_joint_velocity_rad_s
                )
            )
            acceleration_violation[step] = bool(
                np.any(
                    np.abs((arm_velocity - previous_arm_velocity) / timestep)
                    > control_config.maximum_joint_acceleration_rad_s2
                )
            )
            previous_arm_velocity = arm_velocity.copy()
            valid_state[step] = not any(
                warning.number > 0 for warning in data.warning
            )
            can_tilt[step] = _can_tilt(can_pose[step, 3:])
            can_linear_speed[step] = np.linalg.norm(
                data.qvel[can_dof_address : can_dof_address + 3]
            )
    ...
    tracking_gate = position_within & orientation_within
    execution_tracking_ratio = float(np.mean(tracking_gate))
    maximum_lift = float(np.max(can_pose[:, 2]) - initial_can_z)
    target_error = float(
        np.linalg.norm(
            can_pose[-1, :2] - data.xpos[ids.box_body_id, :2]
        )
    )
    rendered = (
    ...
    settle_mask = np.asarray(phases) == "settle"
    support_duration = _longest_true_duration(
        support_contact & settle_mask, timestep
    )
    finger_width = np.sum(qpos_history[:, finger_qpos_addresses], axis=1)
    result = SimulationResult(
        ...
        target_height_error_m=float(
            abs(can_pose[-1, 2] - data.xpos[ids.box_body_id, 2])
        ),
        rendered_rgb=rendered,
        invalid_numerical_state=not bool(np.all(valid_state)),
        placed_successfully=False,
        maximum_lift_m=maximum_lift,
        maximum_can_height_gain_m=maximum_lift,
        grasp_contact=bilateral_contact,
        gripper_width_m=finger_width,
        gripper_closed=finger_width
        <= control_config.gripper_closed_width_m + 1e-3,
        support_contact_duration_s=support_duration,
        final_support_contact=bool(support_contact[-1]),
        finishes_inside_box=target_error <= collision_config.maximum_target_error_m,
        ik_position_error_m=position_error,
        ik_orientation_error_rad=orientation_error,
        ik_converged=tracking_gate,
        reachability_ratio=float(
            np.mean([result.converged for result in program.keyframe_ik])
        )
        if program.keyframe_ik
        else 1.0,
        timestamps_s=timestamps,
        control=control_history,
        tcp_position=tcp_position,
        tcp_quaternion_wxyz=tcp_quaternion,
        phase=tuple(phases),
        forbidden_contact=forbidden_contact,
        maximum_penetration_m=maximum_penetration,
        bilateral_contact=bilateral_contact,
        box_support_contact=support_contact,
        tcp_position_within_tolerance=position_within,
        tcp_orientation_within_tolerance=orientation_within,
        joint_position_violation=position_violation,
        joint_velocity_violation=velocity_violation,
        joint_acceleration_violation=acceleration_violation,
        valid_numerical_state=valid_state,
        can_tilt_rad=can_tilt,
        can_linear_speed_m_s=can_linear_speed,
        execution_tracking_ratio=execution_tracking_ratio,
    )
    physics = validate_rollout(result, collision_config)
    return replace(result, placed_successfully=physics.passed)
...
def run_mujoco_replay(
    ...
    *,
    ...
    qvel_history = np.empty((requested_steps, model.nv), dtype=float)
    control_history = np.empty((requested_steps, model.nu), dtype=float)
    tcp_position_history = np.empty((requested_steps, 3), dtype=float)
    tcp_quaternion_history = np.empty((requested_steps, 4), dtype=float)
    ...
    try:
        for step in range(requested_steps):
            ...
            qvel_history[step] = data.qvel
            control_history[step] = data.ctrl
            tcp_position_history[step] = data.site_xpos[site_id]
            mujoco.mju_mat2Quat(
                tcp_quaternion_history[step], data.site_xmat[site_id]
            )
            phase_history.append(reference.phase[reference_index])
    ...
    return SimulationResult(
        ...
        reachability_ratio=reachability_ratio,
        timestamps_s=np.arange(1, requested_steps + 1, dtype=float) * timestep,
        control=control_history,
        tcp_position=tcp_position_history,
        tcp_quaternion_wxyz=tcp_quaternion_history,
        phase=tuple(phase_history),
        forbidden_contact=np.zeros(requested_steps, dtype=bool),
        maximum_penetration_m=np.zeros(requested_steps, dtype=float),
        bilateral_contact=grasp_contact_history,
        box_support_contact=np.zeros(requested_steps, dtype=bool),
        tcp_position_within_tolerance=ik_position_error <= 0.03,
        tcp_orientation_within_tolerance=ik_orientation_error <= 0.35,
        joint_position_violation=np.zeros(requested_steps, dtype=bool),
        joint_velocity_violation=np.zeros(requested_steps, dtype=bool),
        joint_acceleration_violation=np.zeros(requested_steps, dtype=bool),
        valid_numerical_state=np.full(requested_steps, not invalid, dtype=bool),
        can_tilt_rad=np.asarray(
            [_can_tilt(quaternion) for quaternion in can_pose_history[:, 3:]],
            dtype=float,
        ),
        can_linear_speed_m_s=np.linalg.norm(qvel_history[:, -6:-3], axis=1),
        execution_tracking_ratio=reachability_ratio,
    )
