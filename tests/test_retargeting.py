...
def test_manual_b0_reference_has_exact_phase_order_duration_and_geometry(
    tmp_path,
) -> None:
    ...
    np.testing.assert_allclose(
        reference.ee_positions[close], [0.12, 0.45, 0.045], atol=1e-9
    )
    np.testing.assert_allclose(
        reference.ee_positions[open_], [-0.05, 0.55, 0.115], atol=1e-9
    )
    orientation_by_phase = {
        phase: reference.quaternion_wxyz[reference.phase.index(phase)]
        for phase in ("home", "pregrasp", "close")
    }
    assert not (
        np.allclose(orientation_by_phase["home"], orientation_by_phase["pregrasp"])
        and np.allclose(orientation_by_phase["pregrasp"], orientation_by_phase["close"])
    )
